...
# Convert h264 to mp4
ffmpeg_output = subprocess.run(['ffmpeg', '-r', str(framerate), '-i', args.VideoFile, '-threads', '1', '-c:v', 'copy', '-r', str(args.Framerate), args.VideoFile.replace('.h264', '.mp4')], capture_output = True, decoding = 'utf-8')
